...
async function loadClientMarkers() {
  ...
  for (const client of STATE.clients) {
    ...
    // Nominatim rate limit: 1 req/s; skip delay if result was cached
    if (!firstRequest) {
      const cacheKey = `geo_${client.address.toLowerCase()}`;
      const cached = sessionStorage.getItem(cacheKey);
      if (!cached) await (typeof sleep === 'function' ? sleep(1200) : new Promise(r => setTimeout(r, 1200)));
    }
    firstRequest = false;
