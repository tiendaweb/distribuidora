/* ============================================================
   UTILITY FUNCTIONS
   Funciones compartidas de bajo nivel usadas por toda la app.
   PERSISTENCIA: apiFetch() → wrapper sobre fetch con JSON.
     loadPersistedData() → carga todo el STATE desde /api/bootstrap.
     persistData()       → guarda STATE completo via PUT /api/state.
     save*Api()          → guarda entidades individuales (más seguro
                           que persistData() para cambios únicos).
   IMPORTANTE: persistData() hace un REPLACE TOTAL en el backend.
     Si STATE.products/orders/invoices está vacío por error, los datos
     de la DB se borran. El guard a continuación lo previene.
   ============================================================ */
...
async function loadPersistedData() {
  ...
  try {
    ...
    console.log('Defaults cargados:', { products: defaults.products.length, clients: defaults.clients.length, slides: defaults.slides.length });

    const state = await apiFetch('/api/bootstrap');
    console.log('Bootstrap API respuesta:', { products: state?.products?.length, clients: state?.clients?.length, slides: state?.slides?.length, price_lists: state?.price_lists?.length });
    ...
    STATE.uploadedImages = Array.isArray(state?.images) ? state.images : [];

    STATE.priceLists = Array.isArray(state?.price_lists) && state.price_lists.length
      ? state.price_lists.map(pl => ({ ...pl, overrides: Array.isArray(pl.overrides) ? {} : (pl.overrides || {}) }))
      : [{ id: 1, name: 'Principal', description: 'Lista de precios estándar', factor: 1.0, is_active: 1, is_default: 1, overrides: {} }];
    console.log(`Price lists cargados: ${STATE.priceLists.length}`);

    STATE.activePriceListId = STATE.priceLists.find(pl => pl.is_default)?.id || null;
    console.log(`Active price list ID: ${STATE.activePriceListId}`);

    STATE.changeLog = Array.isArray(state?.change_log) ? state.change_log : [];

    console.log('Datos persistidos cargados completamente');
  } catch (err) {
    ...
    STATE.priceLists = [{ id: 1, name: 'Principal', description: 'Lista de precios estándar', factor: 1.0, is_active: 1, is_default: 1 }];
    STATE.activePriceListId = 1;
    STATE.changeLog = [];
  }
}

async function persistData() {
  // Guard: no reemplazar con arrays vacíos. Si products está vacío,
  // es probable que loadPersistedData() aún no terminó o falló.
  // Esto previene un wipeout accidental de la base de datos.
  if (!STATE.products.length && !STATE.clients.length) {
    console.warn('persistData() abortado: STATE no tiene productos ni clientes (carga incompleta)');
    return;
  }
  try {
...
/**
 * Convierte cualquier string de fecha al formato YYYY-MM-DD.
 * Soporta formato ISO (2025-04-25) y formato es-AR (25/4/2025 14:30:00).
 * Necesario porque getCurrentDateTime() guarda en formato locale, no ISO.
 */
...
// Geolocation — with sessionStorage cache to avoid Nominatim rate limits
async function geocodeAddress(address) {
  try {
    ...
    const cacheKey = `geo_${fullAddress.toLowerCase()}`;
...
// Change audit log
function logChange({ type, productId, productName, field, oldValue, newValue, note } = {}) {
